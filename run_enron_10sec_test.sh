...
RESULT_DIR=~/vf3lib/results/enron_1sec_test/run_$(date +%Y%m%d_%H%M%S)
mkdir -p "$RESULT_DIR"
cd "$RESULT_DIR"

VF3P=~/vf3lib/bin/vf3p
ENRON_DIR=~/vf3_test_enron_NEW
CSV_FILE="enron_1sec.csv"
...
run_test() {
    ...
    echo "Running: $type ${size}v with $threads threads (1 second)..."
    
    /usr/bin/time -v -o "$TIME_FILE" timeout 10s \
...
echo ""
echo "ENRON 1-second test complete! Results:"
cat "$CSV_FILE"
